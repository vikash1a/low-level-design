...
buy(username, stockId, quantity)
sell(username, stockId, quantity)
showPrice(): {stockId, Price}

executionJob()
settlementJob()


user(username, cashAmount, portfolio)
transaction(id, type, user, stock, quantity, time, status)
portfolio(user, list<stock, quantity>)
```